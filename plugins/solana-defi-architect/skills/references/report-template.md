...
- Cluster (e.g. "Anchor — Solana devnet", or the resolved cluster if a custom one was used)
- Prepared for: `<client/project name>`
- Prepared by: name whoever is running this (a person, a team name, or "Claude via the solana-defi-architect skill" if no other identity was given — don't fabricate a firm name)
...
|---|---|---|---|
| SOL-01 | Missing balance check in withdraw() | 🔴 Critical | Unresolved |
| SOL-02 | Unvalidated pool_vault account | 🟠 High | Acknowledged |
| ... | | | |
...
```
### [SOL-01] Missing balance check in withdraw() allows unauthorized fund drain
**Severity:** Critical (impact: High — direct fund loss; likelihood: High — any signer can
call withdraw() for an arbitrary amount regardless of prior deposits)
**Location:** lib.rs, withdraw(), lines 28-36
**Status:** Unresolved
