...
- Chain/ecosystem (e.g. "CosmWasm — ZIGChain testnet", or the target chain if a custom `--chain-docs` was used)
- Prepared for: `<client/project name>`
- Prepared by: name whoever is running this (a person, a team name, or "Claude via the cosmwasm-defi-architect skill" if no other identity was given — don't fabricate a firm name)
...
|---|---|---|---|
| CW-01 | Missing balance check in Withdraw | 🔴 Critical | Unresolved |
| CW-02 | Unchecked arithmetic on Uint128 | 🟠 Medium | Acknowledged |
| ... | | | |
...
```
### [CW-01] Missing balance check in Withdraw allows unbacked withdrawal
**Severity:** Critical (impact: High — accounting corruption/fund loss; likelihood: High — no
special access required, exploitable by any caller regardless of prior deposits)
**Location:** lending.rs, execute_withdraw(), lines 64-72
**Status:** Unresolved
