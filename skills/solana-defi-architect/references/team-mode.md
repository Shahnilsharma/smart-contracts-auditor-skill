...
|---|---|---|
| architect | Scope, threat model, risk matrix, upgrade-authority resilience rating, architecture doc (account/PDA model) | (goes first) |
| contract-engineer | Implements the Anchor program against the architecture doc, following `references/coding-standards-anchor.md` | architect |
| qa-fuzzer | LiteSVM/Mollusk unit tests, Surfpool integration test, Trident/`anchor fuzz` campaign (`references/testing-solana.md`) | contract-engineer |
| static-analyst | Manual pattern scan (`references/audit-checklist.md`) + `cargo audit` | contract-engineer (runs in parallel with qa-fuzzer) |
